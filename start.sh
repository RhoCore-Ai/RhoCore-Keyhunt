...
# Funktion zur Erkennung der Compute Capability für verschiedene GPU-Serien
detect_compute_capability() {
    ...
    # Prüfung auf verschiedene GPU-Serien basierend auf Compute Capability
...
compile_keyhunt() {
    ...
    case $CCAP in
        120)
            echo "Kompilierung für RTX 50XX Series (Compute Capability 12.0)"
            ;;
        89)
            echo "Kompilierung für RTX 40XX Series (Compute Capability 8.9)"
            ;;
        86)
            echo "Kompilierung für RTX 30XX Series (Compute Capability 8.6)"
            ;;
        75)
            echo "Kompilierung für RTX 20XX Series (Compute Capability 7.5)"
            ;;
        *)
            echo "Kompilierung für unbekannte GPU mit Compute Capability $CCAP"
            ;;
    esac
